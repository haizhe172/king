            ...
            <section class="col-lg-12 connectedSortable">
                ...
                <div class="box box-info">
                    ...
                    <div class="box-footer clearfix">
                        <form class="form-horizontal" role="form"  action="/admin/goodsattr/updDo?id={{$data->attr_id}}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            @foreach($res as $k=>$v)
                            <option value="{{$v->type_id}}" @if($data->type_id == $v->type_id) selected @endif>{{$v->cat_name}}</option>
                            @endforeach
                            </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                            <input type="radio" name="attr_type" value="0" {{$data->attr_type=="0" ? "checked" : ""}}>属性
                            <input type="radio" name="attr_type" value="1" {{$data->attr_type=="1" ? "checked" : ""}}>规格
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="firstname" class="col-sm-2 control-label">录入方式</label>
                            <div class="col-sm-10">
                            <input type="radio" name="attr_input_type" value="0"  {{$data->attr_type=="0" ? "checked" : ""}}>手工录入
                            <input type="radio" name="attr_input_type" value="1" {{$data->attr_type=="1" ? "checked" : ""}}>从下面的列表中选择(一行代表一个值)
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="lastname" class="col-sm-2 control-label">可选值列表</label>
                            <div class="col-sm-10">
                            <textarea class="form-control" rows="3" name="attr_values" disabled="disabled">{{$data->attr_values}}</textarea>
                            </div>
                        </div>
