                            ...
                            <div class="tab-pane active" id="home">
                                <div class="row data-type">
		                           ...
		                           <div class="col-md-10 data">
		                               <input type="text" class="form-control" name="act_name" id="act_name"   placeholder="优惠券名称" value="{{$res->act_name}}">
		                           </div>
                                ...
                                <div class="col-md-10 data">
                                    <input  class="form-control" id="meeting" value="{{date('Y-m-d',$res->start_time).'T'.date('H:i:s',$res->start_time)}}" name="start_time" type="datetime-local"/>
                                </div>
                                </div>
                                <div class="row data-type">
                                <div class="col-md-2 title">结束时间</div>
                                <div class="col-md-10 data">
                                    <input  class="form-control" id="meeting" name="end_time" type="datetime-local" value="{{date('Y-m-d',$res->end_time).'T'.date('H:i:s',$res->end_time)}}"/>
                                </div>
                                </div>
                                        ...
                                        <tr>
                                            <td>
                                                <select class="form-control" name="act_range" id="act_range">
                                                    <option value="0" {{$res->act_range==0 ? "selected" : ""}}>全部商品</option>
                                                    <option value="2" {{$res->act_range==2 ? "selected" : ""}}>以下品牌</option>
                                                    <option value="1" {{$res->act_range==1 ? "selected" : ""}}>以下分类</option>
                                                    <option value="3" {{$res->act_range==3 ? "selected" : ""}}>以下商品</option>
                                                </select>
                                            </td>
                                        </tr>
                                    </table>
                                ...
                                <div class="col-md-10 data">
                                    <input type="text" class="form-control" name="min_amount" id="min_amount"   placeholder="金额上限" value="{{$res->min_amount}}">
                                </div>
                                </div>
                                ...
                                <div class="col-md-10 data">
                                    <input type="text" class="form-control" name="max_amount" id="max_amount"   placeholder="0代表没上线" value="{{$res->max_amount}}">
                                </div>
                                </div>
                                        ...
                                        <tr>
                                            <td>
                                                <select class="form-control" name="act_type" id="act_type">
                                                    <option value="">--请选择--</option>
                                                    <option value="1"{{$res->act_type==1 ? "selected" : ""}}>享受赠品</option>
                                                    <option value="2"{{$res->act_type==2 ? "selected" : ""}}>享受现金减免</option>
                                                    <option value="3"{{$res->act_type==3 ? "selected" : ""}}>享受价格折扣</option>
                                                </select>
                                            </td>
                                        </tr>
                                    </table>
                                       <input type="text" class="form-control" name="act_type_ext" id="act_type_ext" style="width:230px; margin-top:-35px; margin-left:150px;"  placeholder="金额" value="{{$res->act_type_ext}}">
...
<script>
   $(function(){
       ...
       var _val = {{$res->act_range_ext}}
       $.get("/admin/coupon/updsou",{act_range:act_range,_val:_val},function(res){
            // console.log(res["result"][0]["cate_id"]);
            if(res["code"]=="00001"){
                alert(res["status"]);
            }
            if(res["code"]=="00000"){
                $(".xuanze").show();
                $(".add").prepend('<input type="checkbox" name="act_range_ext" id="act_range_ext" value="'+res["result"][0]["cate_id"]+'" checked>'+res["result"][0]["cate_name"]);
            }
        })
       ...
       $(document).on("click","#but",function(){
           ...
           var timeout3 = end.replace('T', ' ') +":00";     // 2017-12-07 12:12:00
            ...
            $.post("/admin/coupon/updDo",{data:data},function(res){
                // console.log(res);
                if(res["status"]=="false"){
                    alert(res["msg"]);
                }else{
